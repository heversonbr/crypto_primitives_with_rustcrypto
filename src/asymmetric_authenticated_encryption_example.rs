...
pub fn rsa_oaep_asymmetric_encryption_confidentiality_example() {
    ...
    let dest_private_key = RsaPrivateKey::new(&mut rng, 2048).expect("Failed to generate private key");
    ...
    let encrypted_data = dest_public_key.encrypt(&mut rng, padding, plaintext_msg).expect("Failed to encrypt message");
    debug!("Encrypted Msg: {:?}", &encrypted_data);

    // 4) Decrypt the message with the private key
    let dest_padding = Oaep::new::<Sha256>();
    let decrypted_data = dest_private_key.decrypt(dest_padding, &encrypted_data).expect("Failed to decrypt message!");
...
pub fn rsa_asymmetric_encryption_digital_signatures_authentication(){ 
    ...
    let signer_private_key = RsaPrivateKey::new(&mut rng, 2048).expect("Failed to generate private key");
    let signer_public_key = RsaPublicKey::from(&signer_private_key);

    // Message to be signed
    let message = b"Secure message with RSA Signature";
    info!("Plaintext message: {:?}", std::str::from_utf8(message).expect("Error extracting string from bytes"));
    ...
    let signature = signer_private_key.sign(padding.clone(), &hashed_message)
    ...
    let verify_result = signer_public_key.verify(padding, &hashed_message, &signature);
    match verify_result {
